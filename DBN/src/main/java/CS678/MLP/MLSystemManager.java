...
public class MLSystemManager {
	...
	public SupervisedLearner getLearner(String model, Random rand) throws Exception
	{
		...
		if (model.equals("neuralnet")) return new NeuralNet(rand, 1, numHiddenNodes);
	...
	public void run(String[] args) throws Exception {
		...
		// Set the ARFF file name
		learner.setFileName(fileName);
		
		// Set evaluation method
		learner.setEvaluationMethod(evalMethod);
		
		// Load the ARFF/Serialized data file
		...
		else if (evalMethod.equals("static"))
		{
			...
			if(evalParameter.contains("arff"))
				testData.loadArff(evalParameter);
			else{
				ObjectInputStream ois = new ObjectInputStream(new FileInputStream(evalParameter));
				testData = (Matrix) ois.readObject();
				ois.close();
			}
